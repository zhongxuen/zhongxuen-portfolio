 ...
 *
 * It reads size and nothing else — a checkout's mtimes are the checkout's, so
 * the file's real last-modified date is only knowable from its commit history.
 * The admin console gets it from `recentCommits({ path })` instead.
 */
